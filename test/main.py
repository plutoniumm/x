from numpy import pi, sqrt, round, exp
...
def Hermite(n):
  ...
  if n > 1:
    ...
    return lambda y: 2*y*Hermite(n-1)(y) - 2*(n-1)*Hermite(n-2)(y)
...
def psi(n):
  return lambda x: (alpha(n)/pi)**(1/4) * 1/ sqrt(2**n * fact(n)) * Hermite(n)((x*alpha(n)**0.5)) * exp(-1*(x*alpha(n)**0.5)**2/2)
...
def E(x, n):
  return J2EV(((-1*(hc**2)/(2*m))*psidd(n)(x)/psi(n)(x)) + (1/2)*m*w(n)**2*x**2)
